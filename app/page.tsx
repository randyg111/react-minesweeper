...
// optimize mine generation, win/lose calculation, retry after win, timer start after mine revealed
// github page
...
function Board({squares, hiddenSquares, onPlay, flags, clicks, rightClick, checkSafeStart}: BoardProps) {
  ...
  const board = squares.map((row, r) => {
    ...
    return <div key={r} className="flex">{boardRow}</div>
  })

  return (
    <div className={"h-" + (8*board.length)}>
      {board}
    </div>
  );
}
...
export default function Game() {
  ...
  const bestTimes = (function() {
    ...
    const times = vals.map((val, i) => {
      return <div key={i}>Best {val} time{safeDisplay}: {getLocalStorage("best"+val+"Time"+safe)}</div>
    });
    return (
      <>
        {times}
  ...
  return (
    <div className="grid grid-rows-8 justify-center">
      ...
      <div className="flex justify-center">
        <label>
          Safe Start: <input checked={safeStart} onChange={() => {setSafeStart(!safeStart); retry();}} type="checkbox" />
        </label>
      </div>
    </div>
  )
